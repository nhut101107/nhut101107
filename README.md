...
viptime = 100
ALLOWED_GROUP_ID = -1001931537243   # ID BOX
admin_diggory = "mnhutdznecon" # ví dụ : để user name admin là @diggory347 bỏ dấu @ đi là đc
name_bot = "mnhutspamsmsvip"
zalo = "..."
web = "..."
facebook = "..."
allowed_group_id = -1001931537243 # ID BOX
...
share_log = []
tool = '...'
BOT_LINK = 'https://t.me/mnhutdz_bot'
TOKEN = '7352828711:AAEM-kWD-A8PXrjpYKLbHAn-MRVXKMzzmK0'  
bot = TeleBot(TOKEN)
...
def send_tool_links(message):
    ...
    tool_links = [
        ("https://www.facebook.com/profile.php?id=100072785693898"),
        ("..."),
        ("...")
    ]
    ...
    bot.reply_to(message, "Chọn một  từ bên dưới(...):", reply_markup=markup)
####
#####
...
def share(message):
    ...
    if chat_id != ALLOWED_GROUP_ID:
        msg = bot.reply_to(message, 'Có Cái Lồn')
        time.sleep(10)
        try:
            bot.delete_message(chat_id=message.chat.id, message_id=msg.message_id)
        except telebot.apihelper.ApiTelegramException as e:
    ...
    try:
        ...
        if user_id not in allowed_users and user_id not in freeuser:
            bot.reply_to(message, 'bot chỉ hoạt động khi bạn mua key ')
            return
        if len(args) != 3:
...
- DÙNG LỆNH /key {{key}} ĐỂ TIẾP TỤC -
 [Lưu ý: mỗi key chỉ có 1 người dùng]
    ...
    admin_button = InlineKeyboardButton(text="WED TẢI ALL TOOL", url="...")
...
def key(message):
    if len(message.text.split()) == 1:
        bot.reply_to(message, 'mua đi đéo free')
        return
...
def handle_code_command(message):
    ...
    if len(command_args) < 2:
        bot.reply_to(message, "Vui lòng cung cấp url sau lệnh /code. Ví dụ: /code https://m.facebook.com")
        return
...
│» /spam : Spam SMS FREE
│» /spamvip : Spam SMS VIP - Mua Vip 50k/spam nạn nhân tới chết☠️
...
def spam(message):
    ...
    if len(params) != 2:
        bot.reply_to(message, "/spam sdt số_lần như này cơ mà - free nên đợi 100giây nữa dùng lại nhé")
        return
...
└─────────────
    ...
    script_filename = "smsv1.py"  # Tên file Python trong cùng thư mục
    try:
...
blacklist = ["112", "113", "114", "115", "116", "117", "0359855780", "119", "0", "1", "2", "3", "4"]
...
def supersms(message):
    ...
    if len(params) != 2:
        bot.reply_to(message, "/spamvip sdt số_lần ngu ơi ")
        return
    ...
    if sdt in blacklist:
        bot.reply_to(message, f"Ban Chết Đĩ Mẹ Mày Giờ.")
        return
